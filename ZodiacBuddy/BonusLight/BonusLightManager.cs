...
public class BonusLightManager : IDisposable
{
    ...
    public BonusLightManager()
    {
        ...
        encoder = new JwtEncoder(new HS256Algorithm(algoKey));
        ...
        resetTimer = new Timer(_ => ResetBonus(), null, delta, TimeSpan.FromHours(2));
    }
    ...
    public void AddLightBonus(uint territoryId, DateTime? detectionTime, bool onDutyFromBeginning, string message)
    {
        ...
        if (!ReportStillActive(reportTime))
        {
            return;
        }
        ...
        SendReport(territoryId, (DateTime)detectionTime);
    }
    ...
    private void SendReport(uint territoryId, DateTime detectionTime)
    {
        ...
        request.Headers.Add("x-access-token", GenerateJWT());
    ...
    {
        checkTimer?.Dispose();
        checkTimer = new Timer(_ => RetrieveLastReport(), null, TimeSpan.FromSeconds(2), TimeSpan.FromMinutes(5));
    }
    ...
    private void OnLastReportResponse(string content)
    {
        ...
        {
            NotifyLightBonus(listUpdated.ToArray());
        }
    }
    ...
    private void Send(HttpRequestMessage request, Action<string>? successCallback)
    {
        Task.Run(async () =>
        {
            try
            {
                ...
                LastRequestIsSuccess = response.IsSuccessStatusCode;
    ...
    private string GenerateJWT()
    {
        ...
        return encoder.Encode(payload, TimeSpan.FromMinutes(15));
    }
}
